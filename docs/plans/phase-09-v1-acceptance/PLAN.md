...
会话创建交互采用「空白态 / 已开始才新建」语义：点击 ➕ 进入空白聊天区（侧栏顶部唯一「新对话」占位项），未发送首条消息时再点 ➕ 不新建会话；已存在至少一条用户消息后点 ➕ 才创建新会话。会话标题由 AI 在首轮回答结束后异步生成（首轮完整对话为来源，一句话、中文 10～20 字），生成前显示占位「新对话」，失败静默回退首条 User Message 截断，可手动改名。不引入时间窗口限流或 debounce。详见 ADR 0004。
...
以上范围不含会话创建体验本身：本阶段**包含**「AI 自动生成会话标题 + ➕ 空白态/已开始才新建」的交互与限流语义（见 ADR 0004），属会话创建交互的既定改进；会话标题 AI 生成是显式新增能力（Utility Model 调用、可重试、失败回退首条消息截断），必须在 Phase 09 完成并纳入验收。
