...
import knowflow.sanjin.modules.document.exception.DocumentParseException;
...
public class DocumentParser {
  ...
  public ParsedDocument parse(InputStream in, String filename, boolean isMarkdown)
      throws DocumentParseException {
    String normalized = readNormalized(in);
    if (isMarkdown) {
      return parseMarkdown(normalized, filename);
    }
    return new ParsedDocument(normalized, titleFromFilename(filename));
  }

  private String readNormalized(InputStream in) throws DocumentParseException {
    try {
      byte[] bytes = in.readAllBytes();
      return MimeDetectionService.normalizeUtf8(bytes);
    } catch (IOException e) {
      throw new DocumentParseException("读取文件失败：" + e.getMessage());
    }
  }

  private ParsedDocument parseMarkdown(String markdown, String filename) {
    ...
    return new ParsedDocument(text, title);
  }
  ...
  /** 解析结果：规范正文 + 建议 title。 */
  public record ParsedDocument(String content, String title) {}
}
