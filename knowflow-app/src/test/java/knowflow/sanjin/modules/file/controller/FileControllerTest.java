...
/** FileController 单元测试：下载 owner 校验/缺失、multipart 超限映射、字符串 ID 序列化。 */
class FileControllerTest {
  ...
  void setUp() {
    ...
    FileUploadCoordinator coordinator = mock(FileUploadCoordinator.class);
    mvc =
        MockMvcBuilders.standaloneSetup(
                new FileController(service, coordinator), new MultipartOverflowProbe())
  ...
  void downloadSerializesStringId() throws Exception {
    ...
    mvc.perform(get("/api/v1/files/{id}", "42"))
        ...
        .andExpect(jsonPath("$.knowledgeDocumentId").value("7"))
